    ...
    <>
      {itemParaApagar && (
        <SenhaModal
          ...
          descricao={`Apagar venda de ${itemParaApagar.desc}?`}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  {item.tipo === 'compra' && item.vendaId && (
                    <button
                      ...
                      style={{ background: '#FFEBEE', color: '#D32F2F', border: 'none', borderRadius: '6px', padding: '4px 8px', fontSize: '11px', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 }}
                    >
                      Apagar
                    </button>
                  )}
      ...
      <div style={{ padding: '16px' }}>
        ...
        <div style={{ position: 'relative', marginBottom: '10px' }}>
          ...
          <input
            ...
            placeholder="0,00"
            style={{ width: '100%', padding: '14px 14px 14px 40px', border: '1.5px solid rgba(0,0,0,0.12)', borderRadius: '10px', fontSize: '22px', fontWeight: 700, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box', color: '#2E7D32' }}
          />
        </div>
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '16px' }}>
          {atalhos.map(v => (
            <button key={v} onClick={() => setValor(v.toFixed(2))} style={{ background: parseFloat(valor) === v ? '#2E7D32' : '#E8F5E9', color: parseFloat(valor) === v ? 'white' : '#2E7D32', border: 'none', borderRadius: '20px', padding: '6px 14px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
              {formatMoeda(v)}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '16px' }}>
          {formas.map(f => (
            <button key={f} onClick={() => setForma(f)} style={{ background: forma === f ? '#2E7D32' : '#E8F5E9', color: forma === f ? 'white' : '#2E7D32', border: 'none', borderRadius: '20px', padding: '8px 16px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
              {f}
            </button>
          ))}
        </div>
        ...
        {adms.length === 0 ? (
          <div style={{ background: '#FFF8E1', border: '1px solid #FBC02D', borderRadius: '10px', padding: '10px 14px', fontSize: '13px', color: '#F57F17', marginBottom: '16px' }}>
            Nenhum ADM cadastrado. Va em Configuracoes para cadastrar.
          </div>
        ) : (
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '16px' }}>
            {adms.map(adm => (
              <button key={adm} onClick={() => setRecebidoPor(adm)} style={{ background: recebidoPor === adm ? '#2E7D32' : '#E8F5E9', color: recebidoPor === adm ? 'white' : '#2E7D32', border: 'none', borderRadius: '20px', padding: '8px 16px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                {adm}
              </button>
            ))}
          </div>
