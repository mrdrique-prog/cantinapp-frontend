      ...
      <div style={{ background: 'white', borderRadius: '20px', padding: '28px 24px', width: '100%', maxWidth: '360px', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        ...
        <div style={{ position: 'relative', marginBottom: '8px' }}>
          <input
            ...
            autoFocus
            style={{ width: '100%', padding: '13px 44px 13px 14px', border: `1.5px solid ${erro ? '#D32F2F' : 'rgba(0,0,0,0.15)'}`, borderRadius: '10px', fontSize: '15px', outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }}
          />
          <button onClick={() => setVisivel(!visivel)} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', color: '#9E9E9E' }}>
            {visivel ? '🙈' : '👁'}
          </button>
        </div>
        {erro && <div style={{ color: '#D32F2F', fontSize: '12px', marginBottom: '12px', padding: '0 2px' }}>⚠️ {erro}</div>}
        <div style={{ display: 'flex', gap: '10px', marginTop: '16px' }}>
          <button onClick={onCancelar} style={{ flex: 1, background: '#f5f5f5', color: '#666', border: 'none', borderRadius: '10px', padding: '13px', fontSize: '14px', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 500 }}>
            Cancelar
          </button>
          <button onClick={handleConfirmar} disabled={carregando} style={{ flex: 2, background: '#2E7D32', color: 'white', border: 'none', borderRadius: '10px', padding: '13px', fontSize: '14px', fontWeight: 700, cursor: carregando ? 'not-allowed' : 'pointer', fontFamily: 'inherit', opacity: carregando ? 0.8 : 1 }}>
